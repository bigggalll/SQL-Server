-- LOGIN: Générer le script de création des droits serveurs
	...
	order by Case when sp.type = 'R' then '' else sp.type end,  sp.principal_id;
...
-- USER: Générer le script de création des droits d'une base de données
	SELECT --@@servername, usr.type_desc, usr.name, 
		...
		Then '	CREATE ROLE ' + QUOTENAME(usr.[name]) collate SQL_Latin1_General_CP1_CI_AS + ' AUTHORIZATION ' + (select QUOTENAME(ua.name collate SQL_Latin1_General_CP1_CI_AS) from sys.database_principals (NOLOCK) ua where ua.principal_id = usr.owning_principal_id) + char(10) 
	...
	outer APPLY (SELECT CASE WHEN perm.state <> 'W' THEN perm.state_desc ELSE 'GRANT' END + SPACE(1) 
		...
		order by Case when usr.type = 'R' then '' else usr.type end, usr.principal_id;

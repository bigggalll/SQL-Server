...
/***************************************************************************************************
-- if you ran aotexport from a different environment then where you collected 
-- AX SQL Data use the following script to fix the tables so that these
-- views will work

EXECUTE [SP_UPDATE_AOTEXPORT_DATA] 
   @SERVERNAME = 'Server_name_from_AOTEXPORT_Environment'
  ,@DATABASENAME = 'DB_Name_from_AOTEXPORT_Environment'
  ,@SERVERNAMEUPDATED = 'Server_name_of_AX_SQL_Server'
  ,@DATABASENAMEUPDATED = 'DB_Name_of_AX_SQL_Server'
...
--AOT configuration issues
			...
			-----------------------------------------------------------------
				...
				ORDER  BY ratioofreads DESC, TABLE_NAME DESC 
			...
			FROM   AX_NUM_SEQUENCES_VW STARTING
				   INNER JOIN AX_NUM_SEQUENCES_VW ENDING
						   ON ENDING.NUMBERSEQUENCE = STARTING.NUMBERSEQUENCE
							  ...
							  AND ENDING.PARTITIONNAME = STARTING.PARTITIONNAME
			WHERE  STARTING.STATS_TIME = '2017-02-14 13:00:00.363'
				   AND ENDING.STATS_TIME = '2017-02-14 16:00:00.467'
			ORDER  BY 7 DESC 

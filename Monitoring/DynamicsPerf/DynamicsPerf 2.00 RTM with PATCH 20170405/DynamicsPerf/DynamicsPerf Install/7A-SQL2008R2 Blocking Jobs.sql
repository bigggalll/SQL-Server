...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'DYNPERF_DEFAULT_TRACE_START', 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'Records all blocking information into a trace file C:\SQLTRACE\DynamicsDefault.TRC. You must edit the steps to change the location of this file. Use Query Blocks - Investigate Blocks.sql in the Performance Analyzer 1.16 for Microsoft Dynamics to analyze this data. If the path is changed, you must edit the definition of BLOCKED_PROCESS_VW to the new path.', 
...
DECLARE
	...
	@TRACE_NAME  		NVARCHAR(40)	= ''DynamicsDefault'', -- Trace name - becomes base of trace file name
...
/*
   No Longer Require the database to exist
...
ELSE
	BEGIN
		IF EXISTS(SELECT database_id FROM sys.databases WHERE name = @DATABASE_NAME)
...
EXEC msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Delete Files', 
		@step_id=2, 
		@cmdexec_success_code=0, 
		@on_success_action=1, 
		@on_fail_action=2, 
		...
		@command=N'DEL C:\SQLTrace\DynamicsDefault.trc
DEL C:\SQLTrace\DynamicsDefault_*.trc
', 
		@database_name=N'master', 
		@flags=0

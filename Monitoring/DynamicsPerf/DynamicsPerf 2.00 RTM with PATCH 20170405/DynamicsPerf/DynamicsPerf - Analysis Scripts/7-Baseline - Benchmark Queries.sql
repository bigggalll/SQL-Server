...
SELECT TOP 100 A.SERVER_NAME,
               A.DATABASE_NAME,
               A.QUERY_HASH,
               A.EXECUTION_COUNT_TODAY AS EXECUTION_COUNT,
               A.BEFORE_AVG_TIME,
               A.CURRENT_AVG_TIME,
               A.[TIME_DIFF(ms)],
               A.[%DECREASE],
               (SELECT TOP 1 QUERY_PLAN_PARSED FROM QUERY_HISTORY_VW QHV WHERE QHV.SERVER_NAME = A.SERVER_NAME AND QHV.DATABASE_NAME = A.DATABASE_NAME
				AND QHV.QUERY_HASH = A.QUERY_HASH AND QHV.QUERY_PLAN_HASH = A.BEFORE_PLAN_HASH AND FLAG = 'M' ORDER BY DATE DESC) AS BEFORE_PARSED_PLAN,
				(SELECT TOP 1 QUERY_PLAN_PARSED FROM QUERY_HISTORY_VW QHV WHERE QHV.SERVER_NAME = A.SERVER_NAME AND QHV.DATABASE_NAME = A.DATABASE_NAME
				AND QHV.QUERY_HASH = A.QUERY_HASH AND QHV.QUERY_PLAN_HASH = A.AFTER_PLAN_HASH AND FLAG = 'M' ORDER BY DATE DESC) AS AFTER_PARSED_PLAN,
               B.QUERY_PLAN AS BEFORE_PLAN,
               C.QUERY_PLAN AS AFTER_PLAN
FROM   (SELECT DISTINCT STARTING.SERVER_NAME,
                        STARTING.DATABASE_NAME,
                        STARTING.QUERY_HASH,
                        STARTING.EXECUTION_COUNT_TODAY,
                        STARTING.AVG_TIME_TODAY_MS  AS BEFORE_AVG_TIME,
                        ENDING.AVG_TIME_TODAY_MS    AS CURRENT_AVG_TIME,
                        ENDING.AVG_TIME_TODAY_MS - STARTING.AVG_TIME_TODAY_MS AS 'TIME_DIFF(ms)',
                        CAST(( ENDING.AVG_TIME_TODAY_MS - STARTING.AVG_TIME_TODAY_MS ) / CASE STARTING.AVG_TIME_TODAY_MS WHEN 0 THEN 1 ELSE STARTING.AVG_TIME_TODAY_MS END * 100 AS DECIMAL(14, 3)) AS '%DECREASE',
                       -- STARTING.SQL_TEXT,
                        STARTING.QUERY_PLAN_HASH AS BEFORE_PLAN_HASH,
                        ENDING.QUERY_PLAN_HASH   AS AFTER_PLAN_HASH
        FROM   QUERY_HISTORY STARTING
               INNER JOIN QUERY_HISTORY ENDING
        ...
        WHERE  STARTING.DATE = '2/1/2017' AND STARTING.FLAG = 'D'
               AND ENDING.DATE = '2/2/2017' AND ENDING.FLAG = 'D'
               AND STARTING.AVG_TIME_TODAY_MS < ENDING.AVG_TIME_TODAY_MS
...
SELECT A.SERVER_NAME,A.DATABASE_NAME,A.QUERY_HASH,A.AVG_TIME,A.SQL_TEXT,B.QUERY_PLAN AS QUERY_PLAN
FROM   (SELECT DISTINCT ENDING.SERVER_NAME,ENDING.DATABASE_NAME,ENDING.QUERY_HASH,ENDING.AVG_ELAPSED_TIME AS AVG_TIME,ENDING.QUERY_PLAN_PARSED AS SQL_TEXT,ENDING.QUERY_PLAN_HASH AS PLAN_HASH
        FROM   QUERY_HISTORY_VW ENDING
        WHERE  ENDING.DATE = '2/2/2017' AND ENDING.FLAG = 'D'
               AND NOT EXISTS (SELECT QUERY_HASH
                               ...
                               WHERE  ENDING.QUERY_HASH = STARTING.QUERY_HASH
                                      ...
                                      AND ENDING.SERVER_NAME = STARTING.SERVER_NAME
                                      AND STARTING.DATE = '2/1/2017' AND STARTING.FLAG = 'D')
               AND ENDING.QUERY_HASH <> 0x0000000000000000) AS A
       CROSS APPLY (SELECT TOP 1 QUERY_PLAN
                    FROM   QUERY_PLANS W1
                    WHERE  W1.QUERY_PLAN_HASH = A.PLAN_HASH
                           AND W1.SERVER_NAME = A.SERVER_NAME
                           AND W1.DATABASE_NAME = A.DATABASE_NAME) AS B
ORDER  BY 3 DESC 
...
FROM   (SELECT DISTINCT STARTING.SERVER_NAME,STARTING.DATABASE_NAME,STARTING.QUERY_HASH,STARTING.AVG_ELAPSED_TIME AS BEFORE_AVG_TIME,STARTING.QUERY_PLAN_PARSED AS SQL_TEXT,STARTING.QUERY_PLAN_HASH AS BEFORE_PLAN_HASH
        FROM   QUERY_HISTORY_VW STARTING
        WHERE  STARTING.DATE = '2/1/2017' AND STARTING.FLAG = 'D'
               AND NOT EXISTS (SELECT QUERY_HASH
                               FROM   QUERY_HISTORY_VW ENDING
...
SELECT SERVER_NAME,
       ...
       SUM(ROWS_DELTA) AS NET_CHANGE_ROWS
FROM   PERF_HOURLY_ROWDATA_VW
--WHERE  STATS_TIME = '2017-10-18 08:00:00.587' -- AND  DATABASE_NAME = 'XXXXXXXX' AND SERVER_NAME = 'XXXXXXXXX'
GROUP  BY STATS_TIME,
          SERVER_NAME,
          DATABASE_NAME,
          TABLE_NAME
...
WHERE  IH.ROW_COUNT > 0 AND PREV.ROW_COUNT > 0
       ...
       AND IH.DATE = '2/2/2017'  AND IH.FLAG = 'D'
       AND PREV.DATE = '1/2/2017' AND PREV.FLAG = 'D'

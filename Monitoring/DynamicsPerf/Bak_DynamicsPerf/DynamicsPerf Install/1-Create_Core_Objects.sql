...
CREATE CLUSTERED INDEX [IX_QUERY_PLANS_PARSED_HASH]
    ON [dbo].[QUERY_PLANS]([SERVER_NAME] ASC, [DATABASE_NAME] ASC, [QUERY_PLAN_HASH] ASC);
...
WITH MyCTE (SERVER_NAME, STATS_TIME, WAIT_TYPE, WAITING_TASKS_COUNT, WAIT_TIME_MS, SIGNAL_WAIT_TIME_MS, RowVersion)
     AS (SELECT WS.SERVER_NAME,
         ...
         FROM   WAIT_STATS WS)
SELECT TOP 100 PERCENT WS.STATS_TIME,
                       WS.WAIT_TYPE,
    ...
    FROM   (SELECT LAST.DATABASE_NAME,
            ...
            WHERE  NOT EXISTS (SELECT INDEX_NAME
                               ...
                               WHERE  LAST.SERVER_NAME = START.SERVER_NAME
                                      ...
                                      AND LAST.FLAG = @END_FLAG)
                   AND START.DATE = @START_DATE
                   AND START.FLAG = @START_FLAG) AS A
...
SELECT @SQL = @SQL + '
  ...
  FROM   sys.dm_exec_requests as r
		...
		WHERE r.session_Id > 50	AND  se.host_name IS NOT NULL			   
...
INSERT INTO [dbo].[DYNPERF_PURGETABLES]([TABLE_NAME],[TIME_COLUMN],[SERVER_NAME_FLAG],[DATABASE_NAME_FLAG],[RETENTION_DAYS])  VALUES ('AX_SYSGLOBALCONFIGURATION' ,'STATS_TIME', 1, 1,14) 
...
INSERT INTO [dbo].[DYNPERF_PURGETABLES]([TABLE_NAME],[TIME_COLUMN],[SERVER_NAME_FLAG],[DATABASE_NAME_FLAG],[RETENTION_DAYS])  VALUES ('CRM_ORGRANIZATION' ,'STATS_TIME', 1, 1,14) 

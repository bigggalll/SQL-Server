...
WITH RG_resource
     ...
     SELECT RG_resource.GroupName
          ...
          , CAST(ROUND(100.0 * (interval_cpu_usage_ms) / (2.0 * 30000.0), 2) AS    FLOAT) AS pct_cpu_use_pool_s
          , CAST(ROUND(100.0 * (interval_cpu_usage_ms) / (select count(*) Nbre from sys.dm_os_schedulers where scheduler_id < 255 * 30000.0), 2) AS    FLOAT) AS pct_cpu_use_server_s
